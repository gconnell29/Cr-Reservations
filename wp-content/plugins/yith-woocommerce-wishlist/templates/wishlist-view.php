...
<div class="full_screen full_screen_favorite_materials"> 

        <?php wc_print_notices(); ?>
        <div class="favorite_materials_header">
    ...
    <table cellspacing="0" cellpadding="0" class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo ( is_user_logged_in() ) ? esc_attr( $wishlist_meta['ID'] ) : '' ?>" data-token="<?php echo ( ! empty( $wishlist_meta['wishlist_token'] ) && is_user_logged_in() ) ? esc_attr( $wishlist_meta['wishlist_token'] ) : '' ?>">
        ...
        <tr>
            ...
            <th class="product-name">
                <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( '', 'yith-woocommerce-wishlist' ) ) ?></span>
            </th>
        ...
        if( count( $wishlist_items ) > 0 ) :
            foreach( $wishlist_items as $item ) :
                ...
                if( $product !== false && $product->exists() ) :
	                ...
	                ?>
                    <ul id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
	                    <?php if( $show_cb ) : ?>
                        ...
                        <td class="product-name">
                            ...
                            <span class="fav-title">Material: </span><a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>"><?php echo apply_filters( 'woocommerce_in_cartproduct_obj_title', $product->get_title(), $product ) ?></a>
            ...
            <tr>
                <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php _e( 'No materials have been added to your list', 'yith-woocommerce-wishlist' ) ?></td>
            </tr>
        <?php
        endif;
